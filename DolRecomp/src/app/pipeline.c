...
#include <stdlib.h>
#include <string.h>
// Needed by the grafted LLVM path, which reports failures with strerror(errno)
...
// backend below -- which carries this fork's own work (native loops, --idle-pc,
// call chaining, entry-point tables) -- is untouched by it.
//
// The graft works because the backend reaches CPUState through offsetof() and
// names its runtime helpers, resolving against THIS fork's cpu.h when compiled
// here. That keeps the inlined mem_read32 and the paired-single chain that the
