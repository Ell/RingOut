// NOTE: this fork calls the second memory region mem2/mem2_size where upstream
// calls it exram/exram_size. Same field, same meaning (Wii MEM2 / GameCube
// ARAM-mapped upper RAM); only the name differs, and offsetof resolves against
// whichever cpu.h this backend is compiled with -- which is the whole reason the
// backend could be grafted onto this fork without touching its CPUState layout.
...
Value *FunctionEmitter::emitGuestLoad(Value *address, Type *resultType,
  ...
  Value *exramSize =
      loadOffset(Type::getInt32Ty(context_), offsetof(CPUState, mem2_size));
  ...
  Value *exram =
      loadOffset(PointerType::getUnqual(context_), offsetof(CPUState, mem2));
