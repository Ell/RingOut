...
--engine-replay-corrected-input-probe executes both normalized verification
passes with the remote player's A bit deliberately toggled. It requires an
exact endpoint and at least one changed poll, proving deterministic changed-
input replay without mutating live scheduler state.
...
if [ "$ENGINE_REPLAY_PROBE" -eq 1 ] || [ "$ENGINE_REPLAY_CORRECTED_INPUT_PROBE" -eq 1 ] ||
   ...
   [ "$SELECTIVE_INPUT_UPDATE_REPLAY_PROBE" -eq 1 ] ||
   [ "$SELECTIVE_INPUT_CORRECTED_REPLAY_PROBE" -eq 1 ]; then
  [ "$HOOK_PROFILE" -eq 1 ] || fail "replay probes require --hook-profile"
  [ $((ENGINE_REPLAY_PROBE + ENGINE_REPLAY_CORRECTED_INPUT_PROBE + UPDATE_REPLAY_PROBE + SELECTIVE_UPDATE_REPLAY_PROBE + SELECTIVE_INPUT_UPDATE_REPLAY_PROBE + SELECTIVE_INPUT_CORRECTED_REPLAY_PROBE)) -eq 1 ] ||
  ...
  if [ "$ENGINE_REPLAY_PROBE" -eq 1 ] || [ "$ENGINE_REPLAY_CORRECTED_INPUT_PROBE" -eq 1 ] ||
    ...
    for peer in host guest; do
      ...
      if [ "$SELECTIVE_INPUT_CORRECTED_REPLAY_PROBE" -eq 1 ]; then
        ...
        grep -Fq '[sc2-engine-replay] enabled mode=selective-update-call begin_pc=0x800095c0 return_pc=0x8001bcb0 ticks=1' "$log" ||
          fail "$peer did not enable the selective SC2 update replay probe"
      elif [ "$UPDATE_REPLAY_PROBE" -eq 1 ]; then
        grep -Fq '[sc2-engine-replay] enabled mode=full-emulator-update-call begin_pc=0x800095c0 return_pc=0x8001bcb0 ticks=1' "$log" ||
      ...
      if [ "$SELECTIVE_UPDATE_REPLAY_PROBE" -eq 0 ] &&
         [ "$SELECTIVE_INPUT_UPDATE_REPLAY_PROBE" -eq 0 ] &&
         [ "$SELECTIVE_INPUT_CORRECTED_REPLAY_PROBE" -eq 0 ]; then
      ...
      if [ "$ENGINE_REPLAY_CORRECTED_INPUT_PROBE" -eq 1 ] ||
         [ "$SELECTIVE_INPUT_CORRECTED_REPLAY_PROBE" -eq 1 ]; then
        corrected_contract='corrected_input=yes perturbed_polls=[1-9][0-9]* corrected_state_bytes=[1-9][0-9]*'
      else
        corrected_contract='corrected_input=no perturbed_polls=0 corrected_state_bytes=0'
      fi
      printf '%s\n' "$result" | grep -Eq "state_match=yes cpu_match=yes tb_remainder_match=yes input_replay_match=yes input_polls=[0-9]+ external_profile_complete=yes external_replay_match=yes external_effects=[0-9]+ $corrected_contract endpoint_bytes=[1-9][0-9]* replay_bytes=[1-9][0-9]* differing_state_bytes=0 first_state_difference=0x00000000 last_state_difference=0x00000000 endpoint_value=0x00 replay_value=0x00 endpoint_tb=[0-9]+ replay_tb=[0-9]+$" ||
...
if [ "$ENGINE_REPLAY_PROBE" -eq 1 ]; then
  ...
  run_env+=(RINGOUT_SC2_ENGINE_REPLAY_PROBE=FULL_EMULATOR_ONE_TICK_CORRECTED_INPUT)
elif [ "$UPDATE_REPLAY_PROBE" -eq 1 ]; then
...

{
  ...
  if [ "$HOOK_PROFILE" -eq 1 ]; then
    ...
    if [ "$SELECTIVE_INPUT_CORRECTED_REPLAY_PROBE" -eq 1 ]; then
      echo "sc2_engine_replay_probe=selective-input-update-corrected"
    elif [ "$ENGINE_REPLAY_CORRECTED_INPUT_PROBE" -eq 1 ]; then
