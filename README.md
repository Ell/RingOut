...
**Netplay**: the `v1.2.1-ell.12` beta adds rollback, an Online Room flow using
...
| `RingOut-1.2.1-ell.12-linux-x86_64.AppImage` | Linux x86-64 rollback beta | yes — compiles on your machine |
| `RingOut-1.2.1-ell.12-windows-x86_64.zip` | Windows 10/11 x86-64 rollback beta | no — toolchain bundled |
...
```sh
chmod +x RingOut-1.2.1-ell.12-linux-x86_64.AppImage
./RingOut-1.2.1-ell.12-linux-x86_64.AppImage
```
...
`RingOut-1.2.1-ell.12-appimage-runtime-sources.tar.zst` release asset available
...
- **Full controller remapping** from the launcher's Settings page, with direct
  button/axis capture, immediate persistence, and one-click default restore
