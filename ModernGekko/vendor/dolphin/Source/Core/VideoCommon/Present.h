...
private:
  ...
  // Serialises whole Present() calls. Normally only the video thread presents,
  // but the in-game settings overlay redraws from the host thread while
  // emulation is paused (RecompMenu::PumpFrame). If the video thread has not
  // fully stopped yet, both run the XFB/ImGui rendering above -- which is
  // outside m_swap_mutex and not thread safe -- and the window wedges. Rare,
  // timing dependent, and it survives many successful opens before biting.
  std::mutex m_present_call_mutex;
