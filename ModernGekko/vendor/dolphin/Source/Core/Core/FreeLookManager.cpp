...
void FreeLookController::LoadDefaults(const ControllerInterface& ciface)
{
  ...
  // The mouse bindings above rely on a ControllerInterface mouse device, which
  // does not exist under a native Wayland NoGUI window (keyboards/pads still
  // arrive via evdev). Add keyboard rotation alongside them so free look is
  // usable without a mouse: Shift+arrows to look, Shift+Z/C to roll. These are
  // OR'd with the mouse expressions rather than replacing them.
  auto add_rotation = [&](GyroButtons::GyroButtons button, const std::string& key) {
    const std::string existing = m_rotation_gyro->controls[button]->control_ref->GetExpression();
    const std::string keyboard = fmt::format("@(Shift+{}) * 0.05", key);
    m_rotation_gyro->SetControlExpression(
        button, existing.empty() ? keyboard : fmt::format("({}) + ({})", existing, keyboard));
  };
  add_rotation(GyroButtons::PitchUp, "Up");
  add_rotation(GyroButtons::PitchDown, "Down");
  add_rotation(GyroButtons::YawLeft, "Left");
  add_rotation(GyroButtons::YawRight, "Right");
  add_rotation(GyroButtons::RollLeft, "Z");
  add_rotation(GyroButtons::RollRight, "C");
#endif
}
