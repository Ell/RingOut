...
void Jit64AsmRoutineManager::Generate()
{
  ...
  // StaticRecomp hand-back: before JIT-compiling a block, if this guest PC is
  // covered by the loaded static-recomp module, exit the dispatcher (RET) so
  // StaticRecompCore::Run() regains control and executes it natively. Covered
  // PCs are therefore never compiled or entered into the fast block map, so the
  // asm fast-lookup can never bypass this. Only emitted for the StaticRecomp
  // fallback JIT; ordinary Jit64 is unaffected.
  Gen::FixupBranch recomp_handback{};
  bool has_recomp_handback = false;
  if (Config::Get(Config::MAIN_CPU_CORE) == PowerPC::CPUCore::StaticRecomp)
  {
    ABI_PushRegistersAndAdjustStack({}, 0);
    MOV(32, R(ABI_PARAM1), PPCSTATE(pc));
    ABI_CallFunction(StaticRecompCoveredAt);
    ABI_PopRegistersAndAdjustStack({}, 0);
    TEST(8, R(ABI_RETURN), R(ABI_RETURN));
    recomp_handback = J_CC(CC_NZ, Jump::Near);
    has_recomp_handback = true;
  }
  ...
  if (has_recomp_handback)
    SetJumpTarget(recomp_handback);
