                                  ...
                                  uint32_t state)
{
  ...
  {
    // The only release anyone cares about: hold-Tab fast-forward.
    if (symbol == XKB_KEY_Tab)
      RecompMenu::SetFastForward(false);
    return;
  }
...
void PlatformWayland::HandleHotkey(xkb_keysym_t symbol)
{
  ...
  // Escape opens the in-game settings overlay (and pauses); it used to quit
  // outright. Quitting now lives in the menu itself, with Shift+Escape kept as
