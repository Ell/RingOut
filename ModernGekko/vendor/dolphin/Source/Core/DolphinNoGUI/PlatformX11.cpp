...
{
  // Fullscreen and quit are window-system operations the overlay can't perform
  // itself, so hand it callbacks into this platform.
  RecompMenu::SetFullscreenCallback([this] {
  ...
  XSelectInput(m_display, m_window,
               StructureNotifyMask | KeyPressMask | KeyReleaseMask | FocusChangeMask);
  // Without this, holding a key delivers synthetic release/press pairs, which
  // would strobe the hold-Tab fast-forward off/on every repeat.
...
void PlatformX11::ProcessEvents()
{
  ...
  for (int num_events = XPending(m_display); num_events > 0; num_events--)
  {
    ...
    {
    case KeyRelease:
      if (XLookupKeysym((XKeyEvent*)&event, 0) == XK_Tab)
        RecompMenu::SetFastForward(false);
      break;
    case KeyPress:
    ...
    case FocusOut:
    {
      ...
      if (Config::Get(Config::MAIN_SHOW_CURSOR) == Config::ShowCursor::Never)
        XUndefineCursor(m_display, m_window);
      // Losing focus mid-hold means the Tab release will never arrive.
      RecompMenu::SetFastForward(false);
    }
    break;
    case ClientMessage:
