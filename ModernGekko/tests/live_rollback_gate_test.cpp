...
bool TestProductionCoverageFailsClosed() {
  Gate gate;
  CHECK(!gate.HasCompleteCoverage());
  CHECK(!gate.BeginHiddenReplay(10, 11));
  ...
  CHECK(!coverage[static_cast<std::size_t>(
      NetPlay::RollbackHostEffect::AudioReconciliation)]);
  CHECK(!coverage[static_cast<std::size_t>(
  ...
  CHECK(!coverage[static_cast<std::size_t>(
      NetPlay::RollbackHostEffect::NetPlayOutbound)]);
  CHECK(!coverage[static_cast<std::size_t>(
  ...
  CHECK(boundary.Activate(100) ==
        Boundary::ActivationStatus::OutputCoverageIncomplete);
  CHECK(store.frames.empty());
  return true;
}
...
bool TestGlobalGateOwnershipAndFaultLatch() {
  ...
  first.EndHiddenReplay(true);
  CHECK(!NetPlay::IsLiveRollbackHiddenReplayActive());

  CHECK(second.BeginHiddenReplay(5, 5));
  second.EndHiddenReplay(false);
...
bool TestFrameBoundaryDeactivateDisarmsHiddenReplay() {
  FakeStateStore store;
  Gate gate(Gate::CompleteCoverageForTesting());
  Coordinator coordinator({.enabled = true, .max_replay_frames = 2}, store,
                          gate);
...
int main() {
  return TestProductionCoverageFailsClosed() &&
                 TestGlobalGateOwnershipAndFaultLatch() &&
                 TestFrameBoundaryCorrectionAndCommit() &&
                 TestFrameBoundaryDeactivateDisarmsHiddenReplay()
             ? 0
             : 1;
}
